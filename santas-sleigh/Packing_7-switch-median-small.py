...
count_not_rotate, count_rotate = 0, 0

class Present:
    def __init__(self, row):
        global count_not_rotate, count_rotate
        present = list(map(int, row))
        present = [present[0]] + sorted(present[1:4])
        self.idx = present[0]
        self.large = present[3]
        if self.large < 71 or present[1] < 65:
            self.median = present[2]
            self.small = present[1]
            # if not in class 4, we always assume small<=median<=large
        #elif present[1] >= 71 or present[2] > 250: 
        #    self.median = present[2]
        #    self.small = present[1]
        #    count_not_rotate += 1
        #    # wsmall<=median<=large
        else:                                           # seems doesn't course improvement
            self.median = present[1]                    #
            self.small = present[2]                     #
            count_rotate += 1                           #
            # lazy switch indices                       #
    def __repr__(self):
        return (str(self.idx) + ': ('
...
with open('presents_small.csv', 'rb') as csvfile:
...
print(count_not_rotate, count_rotate)
