...
void pred_row_label (csr *features, csr *features_copy, int nrow, csr *train_labels, csr *tr_train_features, csr *test_labels) {
	...
	for (i = features->ptr[nrow]; i < features->ptr[nrow+1]; ++i)  {
		if (features_copy->val[i] == 1) continue;
		k = features->idx[i];
...
int main (int argc, char *argv[]) {
	...
	csr *eval_features_copy = csr_malloc(EVAL_NROWS, EVAL_NFEATUREDATA);

	FILE *eval;
	eval = fopen(argv[2], "r");
	read_train(eval, eval_labels, eval_features);
	eval = fopen(argv[2], "r");
	read_train(eval, eval_labels, eval_features_copy);
	fclose(eval);
	...
	for (i = 0; i < eval_features->nrows; ++i) {
		printf("%d,", i+1);
		pred_row_label(eval_features, eval_features_copy, i, train_labels, tr_train_features, eval_labels);
