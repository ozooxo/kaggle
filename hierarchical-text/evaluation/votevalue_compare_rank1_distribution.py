...
for lineF, lineG in izip(f, g):
	...
	for i in range(1,5):
		if labelF[i][0] in labelG.keys():
			true_set[i].append((float(labelF[i][1])+0.00001)/(float(labelF[0][1])+0.00001))
		elif labelF[0][0] in labelG.keys():
			false_set[i].append((float(labelF[i][1])+0.00001)/(float(labelF[0][1])+0.00001))
