...
void reverse_step (int start_grid[][24], int stop_grid[][24], int vote_case[], double probability_case[]) {
	...
	for (i = 2; i < 22; ++i) {
		for (j = 2; j < 22; ++j) {	
			idx = get_pattern_idx(stop_grid, i, j);
			if (stop_grid[i][j] == 0) {
				if (probability_case[idx] > 0.640) start_grid[i][j] = 1;
				else start_grid[i][j] = 0;
			}
			else {
				//if (vote_case[idx] == 1) start_grid[i][j] = 1;
				if (probability_case[idx] > 0.607) start_grid[i][j] = 1;
				else start_grid[i][j] = 0;
			}			
		}
	}
	// fix the "Blinker": right now it is really less efficient, but it works.
	// also, fix that just make the score really worse. so I'll just delete that case.
	/*
	for (i = 2; i < 22; ++i) {
		for (j = 2; j < 22; ++j) {	
			if (get_pattern_idx(stop_grid, i, j) == 0102040 &&
			    stop_grid[i-2][j-2] == 0 && stop_grid[i-2][j+2] == 0 &&
			    stop_grid[i+2][j-2] == 0 && stop_grid[i+2][j+2] == 0) {
				start_grid[i][j-1] = 1;
				//start_grid[i][j] = 1;
				start_grid[i][j+1] = 1;
			...
			else if (get_pattern_idx(stop_grid, i, j) == 07000 &&
			         stop_grid[i-2][j-2] == 0 && stop_grid[i-2][j+2] == 0 &&
			         stop_grid[i+2][j-2] == 0 && stop_grid[i+2][j+2] == 0) {
				start_grid[i-1][j] = 1;
				//start_grid[i][j] = 1;
...
int main () {
	...
	*/
	...
	return 0;
	// use probability (so adjust a more flexible cutoff for voting).
	// finally choose (center0:0.64, center1:0.605). however, in this choice, "Blinker" is missing.
	// training set score: 0.125605 (after fix the "Blinker": 0.144986 -- much worse)
	// real score: 0.12707
	
	// parameter test table
/*
	0.55	0.6	0.62	0.63	0.635	0.639	0.64	0.65	0.66	0.7
0.55							0.126181			
0.59							0.125754			
0.6		0.125721	0.125685	0.125657			0.125638	0.125653		0.125825
0.605					0.125623		0.12561			
0.607						0.125611	0.125605			
0.61				0.125629			0.125622	0.125636	0.125664	
0.62							0.125681			
0.65							0.126105			 
*/	
