...
for trainStr in trainList:
    if (trainStr):
        trainData = map(int,trainStr.split(','))
        gridStop = list2grid(trainData[2:402], edge)
        gridStart = deltaBack(trainData[1], gridStop, infoList)
        g.write(",".join(map(str, trainData[:1]+grid2list(gridStart))))
        g.write('\n')
